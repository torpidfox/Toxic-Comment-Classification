...
json tcc::KaggleDataProvider::_sample_to_json(std::vector<std::string>& s, std::vector<bool>& r){
	...
	std::transform(json_tags.begin(), json_tags.end() - 1, s.begin(), tags_list.begin(),
		std::make_pair<std::string, std::string&>);
	for (auto el : tags_list)
		result[el.first] = el.second;

	classes_list.resize(classes.size());
	std::transform(classes.begin(), classes.end(), r.begin(), classes_list.begin(),
...
std::string tcc::KaggleDataProvider::_parse_id(std::string& line) {
...
bool tcc::KaggleDataProvider::_parse_text(std::string& line, std::string& dst) {
	size_t start = 0;
...
std::vector<bool> tcc::KaggleDataProvider::_parse_rating(std::string& line) {
...
bool  tcc::KaggleDataProvider::save_data(const json& msg) const {
	return true;
}
